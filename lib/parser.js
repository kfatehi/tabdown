...
class Parser {
    ...
    parse(str) {
        ...
        let ip = new IteratedParser(this.options);
        ip.parse(tokens);
        ip.finish();
        return ip.getTree();
    }
...
class Node {
    ...
    add(node, replacer) {
      this.children.push(replacer(node));
    }
    getLines() {
...
class IteratedParser {
    constructor(options) {
      this.replaceNode = options.replaceNode || function(i){ return i };
    ...
    parse(tokens) {
        for (let tok of tokens) {
            if (tok.type == "EXPR") {
                ...
                this.nodeStack[0].add(this.lastNode, this.replaceNode);
            }
